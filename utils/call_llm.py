...
def get_llm_provider():
    ...
    if not provider and (os.getenv("GEMINI_PROJECT_ID") or os.getenv("GEMINI_API_KEY")):
        provider = "GEMINI"
    # If necessary, add ANTHROPIC/OPENAI here
    return provider
...
# Comment out the Gemini call_llm function, opening the OpenAI call_llm function and make the adjustment.
...
def _call_llm_gemini(prompt: str) -> str:
    if os.getenv("GEMINI_PROJECT_ID"):
        ...
        client = genai.Client(api_key=os.getenv("GEMINI_API_KEY"), http_options=types.HttpOptions(base_url=BASE_URL))
    else:
        raise ValueError(
